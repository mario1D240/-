...
if not lowQuality then	
 addLuaScript('data/whopeee/ratings/ratingssasescombo')	
...
function goodNoteHit(id, direction, noteType, isSustainNote)
	...
	if not isSustainNote and resetHideHud and getProperty('combo') then
		...
		makeLuaSprite('lastRating', 'ratings/vanilla/' .. rating, screenWidth * 0.35 - -85 + downscrolloffset , screenHeight / 2 - 200 - downscrolloffset * 3 );
		...
		if combo >= 0 then
			for i=2,0,-1 do
				...
				makeLuaSprite(tag, 'ratings/vanilla/num' .. math.floor(combo / 10 ^ i % 10), screenWidth * 0.35 - -145 - i * 43 + downscrolloffset, screenHeight / 2 + -90 - downscrolloffset * 3 );
