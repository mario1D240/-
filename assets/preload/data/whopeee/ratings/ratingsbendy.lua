...
if not lowQuality then	
 addLuaScript('data/whopeee/ratings/ratingsbendycombo')	
	resetHideHud = not hideHud;
	if resetHideHud then
		setPropertyFromClass('ClientPrefs', 'hideHud', true);
	end
	precacheImage('ratings/bendy/shit');
	precacheImage('ratings/bendy/bad');
	precacheImage('ratings/bendy/good');
	precacheImage('ratings/bendy/sick');
	precacheImage('ratings/bendy/num');
end	
end
...
function goodNoteHit(id, direction, noteType, isSustainNote)
	...
	if not isSustainNote and resetHideHud and getProperty('combo') then
		...
		makeLuaSprite('lastRating', 'ratings/bendy/' .. rating, screenWidth * 0.35 - -85 + downscrolloffset , screenHeight / 2 - 200 - downscrolloffset * 3 );
		...
		if combo >= 0 then
			for i=2,0,-1 do
				...
				makeLuaSprite(tag, 'ratings/bendy/num' .. math.floor(combo / 10 ^ i % 10), screenWidth * 0.35 - -145 - i * 43 + downscrolloffset, screenHeight / 2 + -90 - downscrolloffset * 3 );
